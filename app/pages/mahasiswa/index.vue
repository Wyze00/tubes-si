...
const stats = computed(() => {
  const data = mhsData.value?.data || [];
  const total = data.length;

  const prodiCounts: Record<string, number> = {};
  data.forEach((m: any) => {
    const kode = m.Kd_Prodi;
    if (kode) {
        prodiCounts[kode] = (prodiCounts[kode] || 0) + 1;
    }
  });

  const uniqueProdi = Object.keys(prodiCounts).length;

  let maxCount = 0;
  let topProdiKode = '';
  for (const [kode, count] of Object.entries(prodiCounts)) {
    if (count > maxCount) {
      maxCount = count;
      topProdiKode = kode;
    }
  ...
  const topProdiName = topProdiKode ? getProdiName(topProdiKode) : '-';

  return { total, uniqueProdi, topProdiName, maxCount };
});
      ...
      <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center text-sm text-gray-500">
                ...
                <span>Total Mahasiswa: {{ filteredMhs.length }}</span>
            </div>
        ...
        <div class="mt-4 pt-4 border-t border-gray-200">
            <div class="flex gap-4">
                <div class="flex-1 text-center p-2 hover:bg-gray-50 rounded-lg transition-colors">
                    ...
                    <div class="text-xs text-gray-500 font-medium  mt-1">Total Mahasiswa</div>
                </div>
                <div class="flex-1 text-center p-2 hover:bg-gray-50 rounded-lg transition-colors">
                    <div class="text-2xl font-bold text-blue-600">{{ stats.uniqueProdi }}</div>
                    <div class="text-xs text-gray-500 font-medium  mt-1">Total Prodi</div>
                </div>
                <div class="flex-1 text-center p-2 hover:bg-gray-50 rounded-lg transition-colors">
                    <div class="text-2xl font-bold text-purple-600 truncate px-2" :title="stats.topProdiName">{{ stats.topProdiName }}</div>
                    <div class="text-xs text-gray-500 font-medium  mt-1">Prodi Terpadat ({{ stats.maxCount }})</div>
                </div>
            </div>
        </div>
      </div>
